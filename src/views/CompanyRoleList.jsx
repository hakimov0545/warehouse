...
import { companyRoleApi } from '../api/companyRole'
...
const ACTION_TOKENS = ['CREATE', 'READ', 'UPDATE', 'DELETE', 'MANAGE', 'VIEW']
...
export default function CompanyRoleList() {
  ...
  const columns = useMemo(
    () => [
      { key: 'name', label: t('companyRole.name'), sortable: true, cellClass: 'cell-name' },
      {
        key: 'permissions',
        label: t('companyRole.permissions'),
        render: (_v, row) => <span className="perm-count">{(row.permissions || []).length}</span>
      }
    ],
    [t]
  )
  ...
  const items = useMemo(() => {
    ...
    return allRoles.filter((r) => r.name && r.name.toLowerCase().includes(q))
  }, [search, allRoles])
  ...
  const permissionGroups = useMemo(() => {
    ...
    for (const perm of allPermissions) {
      ...
      const moduleKey = ACTION_TOKENS.includes(last) ? parts.slice(0, -1).join('_') : perm.code
  ...
  async function load() {
    ...
    try {
      ...
      const { data } = await companyRoleApi.getByCompanyId(companyId)
  ...
  async function openForm(item = null) {
    ...
    if (item) {
      ...
      setSelected(new Set((item.permissions || []).map((p) => p.code)))
    } else {
      setForm({ name: '' })
      setSelected(new Set())
    }
  ...
  return (
    <div className="page-view">
      ...
      <AppModal
        ...
        title={editing ? t('companyRole.editTitle') : t('companyRole.addTitle')}
        size="lg"
        footer={
          <>
      ...
      >
        <form onSubmit={handleSubmit} className="modal-form">
          <FormField label={t('companyRole.name')} error={errors.name} required>
            <input
              value={form.name}
          ...
          <div className="perm-section">
            <div className="perm-section-head">
              <label>{t('companyRole.permissions')}</label>
              <div className="perm-section-actions">
                <button type="button" className="link-btn" onClick={selectAll}>{t('companyRole.selectAll')}</button>
                <button type="button" className="link-btn" onClick={clearAll}>{t('companyRole.clearAll')}</button>
              </div>
            </div>
            ...
            {permsLoading ? (
              ...
              <p className="perm-empty">{t('companyRole.noPermissions')}</p>
            ) : (
                    ...
                    <div className="perm-grid">
                      {group.items.map((perm) => (
                        ...
                        >
                          ...
                          />
                          <span className="perm-name">{perm.name || perm.code}</span>
