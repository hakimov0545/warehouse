...
export default function SupplierList() {
  ...
  const columns = useMemo(() => [
    { key: 'name', label: t('supplier.name'), sortable: true, cellClass: 'cell-name' },
    { key: 'inn', label: t('supplier.inn'), sortable: true },
    { key: 'phone', label: t('supplier.phone') },
    { key: 'email', label: t('supplier.email') },
    { key: 'contactPerson', label: t('supplier.contactPerson') }
  ], [t])
  ...
  async function load() {
    setLoading(true)
    try {
      const { data } = await supplierApi.getByCompany(company.id)
