...
export const useAuthStore = create((set, get) => ({
  ...
  companyUser: JSON.parse(localStorage.getItem('companyUser') || 'null'),
  ...
  login: async (credentials) => {
    ...
    try {
      ...
      const companyRole = data.companyRole || data.warehouseRole || null
  ...
  fetchCompanyUser: async () => {
    ...
    try {
      const { data } = await companyUserApi.getByUserIdAndCompanyId(companyId)
  ...
  setPendingCredentials: (credentials) => set({ pendingCredentials: credentials }),
  ...
  register: async (data) => {
    ...
    try {
      ...
      const companyRole = res.data.companyRole || res.data.warehouseRole || null
      localStorage.setItem('token', token)
      if (companyRole) {
        localStorage.setItem('companyRole', companyRole)
      } else {
